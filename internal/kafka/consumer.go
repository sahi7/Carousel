...
func (c *Consumer) Run(handlers map[string]EventHandler) error {
    ...
    for {
        ...
        if err == nil {
            // log.Printf("Received Kafka message: Topic=%s, Key=%s, Value=%s", *msg.TopicPartition.Topic, string(msg.Key), string(msg.Value))
            eventChan <- msg
...
func (c *Consumer) processWorker(eventChan <-chan *kafka.Message, handlers map[string]EventHandler) {
    for msg := range eventChan {
        ...
        if err := json.Unmarshal(msg.Value, &event); err != nil {
            log.Printf("Failed to parse event from message (Topic=%s, Value=%s): %v", *msg.TopicPartition.Topic, string(msg.Value), err)
            c.sendToDLQ(msg)
            continue
        }

        // log.Printf("Processed event: %+v", event) // Log the parsed event struct 
